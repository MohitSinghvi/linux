...
// Assignment 3
u32 exit_count_input[69];
u64 processing_time[69];
EXPORT_SYMBOL(exit_count_input);
EXPORT_SYMBOL(processing_time);
...
//Assignment 2
	...
	//leaf node 0x4ffffffc
	  	...
	  	printk(KERN_INFO "###### ECX: Exit# = %d",(int)ecx);
		if(ecx >= 0 && ecx <= 69){
			if(ecx == 35 || ecx == 38 || ecx == 42 || ecx == 65){
				printk(KERN_INFO "###### Exit number in input is not defined: %d", ecx);
				eax = 0; ebx = 0; ecx = 0; edx = 0xFFFFFFFF;
			}else{
				ebx = ((processing_time[(int)ecx]) >> 32); // high 32 bits
				ecx = (processing_time[(int)ecx] & 0xFFFFFFFF ); // low 32 bits
                printk(KERN_INFO "###### Exit# = %d, Exit Duration = %llu", (int)ecx, processing_time[(int)ecx]);
			}
		}else{
			printk(KERN_INFO "###### Exit number in input is not defined: %d", ecx);
	...
	//leaf node 0x4ffffffd
		...
		if(ecx >= 0 && ecx <= 69){
			if(ecx == 35 || ecx == 38 || ecx == 42 || ecx == 65){
				...
				eax = exit_count_input[(int)ecx];
				printk(KERN_INFO "###### Exit# = %d, Exit Count = %d", (int)ecx, exit_count_input[(int)ecx]);
			}
		}
	}
