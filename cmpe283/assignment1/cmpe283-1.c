 ...
 * See SDM volume 3, section 25.8.1
 */
struct capability_info entry_control[14] =
{
 ...
 */
struct capability_info exit_control[18] =
{
	{ 2, "Save debug controls" },
	{ 9, "Host address space size" },
	{ 12, "Load IA32_PERF_GLOBAL_CTRL" },
 ...
 */
struct capability_info primary_proc_based_control[22] =
{
 ...
 * See SDM volume 3C, section 25.6.2
 */
struct capability_info secondary_proc_based_control[31] =
{
 ...
 * See SDM volume 3C, section 25.6.2
 */
struct capability_info procbasedtertiary[6] =
{
	{ 0, "LOADIWKEY exiting" },
	{ 1, "Enable HLAT" },
...
detect_vmx_features(void)
{
	...
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entry_control, 14, lo, hi);

	/* Exit controls */
	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exit_control, 18, lo, hi);

	/* Primary Processor based controls */
	rdmsr(IA32_VMX_PRIMARY_PROCBASED_CTLS, lo, hi);
	pr_info("Primary Processor based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(primary_proc_based_control, 22, lo, hi);

	/* Secondary Processor based controls */
	rdmsr(IA32_VMX_SECONDARY_PROCBASED_CTLS, lo, hi);
	pr_info("Secondary Processor based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(secondary_proc_based_control, 31, lo, hi);

	/* VM-Tertiary-Procbased Controls */
	rdmsr(IA32_VMX_TERTIARY_PROCBASED_CTLS, lo, hi);
	pr_info("VM-Tertiary-Procbased Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbasedtertiary, 6, lo, hi);
